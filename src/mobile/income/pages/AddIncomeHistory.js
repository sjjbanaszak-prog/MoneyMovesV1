...
export default function AddIncomeHistory() {
  const { idx }                   = useParams();
  ...
  const existingYears = new Set([
    currentYear,
    ...(income.salaryHistory || []).map(h => h.year),
  ]);

  const [year,           setYear]           = useState(String(currentYear - 1));
  const [employmentType, setEmploymentType] = useState(income.employmentType || 'Full-time');
  ...
  const isDupe   = existingYears.has(yearNum);
  ...
  function handleBonusToggle() {
    ...
    if (!bonusEnabled) setBonuses([{ label: '', amount: '' }]);
  }
  ...
  const canSubmit = annualSalary !== '' && !isNaN(yearNum) && yearNum >= 1990 && yearNum <= currentYear && !isSaving && !isDupe;
  ...
  async function handleConfirm() {
    ...
    const existing = income.salaryHistory || [];
    await updateIncome(Number(idx), { salaryHistory: [...existing, newEntry] });
    navigate(`/mobile/income/employer/${idx}`);
  }

  return (
    <IncomeDetailLayout title="Add History" backTo={`/mobile/income/employer/${idx}`}>
      <div style={{ padding: '16px' }}>
        ...
        <div style={{
          ...
          <span className="material-symbols-outlined" style={{ fontSize: '14px', color: '#4edea3' }}>history</span>
          <span style={{ fontSize: '12px', color: '#4edea3', fontWeight: 600 }}>{income.employer}</span>
        </div>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            {isDupe ? (
              <p style={{ fontSize: '11px', color: '#ffb95f', margin: '6px 0 0' }}>
                Data for {yearNum} already exists — edit from the detail page instead.
              </p>
            ) : (
              <p style={{ fontSize: '11px', color: '#64748b', margin: '6px 0 0' }}>
                Enter the calendar year this salary applied to
              </p>
            )}
          </div>
        ...
        <div style={{
          ...
          <p style={{ fontSize: '13px', color: '#bbcabf', margin: 0, lineHeight: 1.55 }}>
            Historical entries build your income chart over time. The current year's figures are managed from the overview page.
          </p>
        </div>
